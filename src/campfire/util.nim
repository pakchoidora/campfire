...
proc getAABB*(line: Line): AABB =
  return getAABB(line[0], line[1])
...
template alias*(a,b: typed): typed =
  template a: typed = b
...
proc drawTiledMap*(tx,ty,tw,th: int, dx,dy, dw,dh: int, ox,oy: int, scale: float = 1.0) =
  ...
  for dy in countup(startY, startY+dh + (drawHeight-1), drawHeight):
    for dx in countup(startX, startX+dw + (drawWidth-1), drawWidth):
      drawMap(tx,ty,tw,th,dx,dy,scale)
