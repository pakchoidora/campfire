...
var controllerAddedFunc: proc(controller: Controller)
var controllerRemovedFunc: proc(controller: Controller)
...
var virtualButtons*: array[2, array[10, seq[PhysicalInput]]] = [
  ...
  [
    # TODO: Support customize keybinding
    @[
      PhysicalInput(kind: Key, index: SDL_SCANCODE_LEFT.int),
    ],
    @[
...
proc getControlName*(pid, btn: int): string =
  ...
  for phys in virtualButtons[pid][btn]:
    if phys.kind == Key:
      bits.add(
        ($getKeyName(getKeyFromScancode(phys.index.Scancode))).toLowerAscii()
      )
    elif phys.kind == JButton:
      bits.add("jbutton" & $(phys.index + 1))
    elif phys.kind == JHat:
      var hatDir: string
      case int(phys.value.uint8):
        of SDL_HAT_LEFT:
          hatDir = "left"
        of SDL_HAT_RIGHT:
          hatDir = "right"
...
proc btn*(b: ControllerButton, player: range[0..7]): bool =
  if player > controllers.high:
    return false
  return controllers[player].btn(b)

proc btnp*(b: ControllerButton, player: range[0..7]): bool =
  if player > controllers.high:
    return false
  return controllers[player].btnp(b)

proc btn*(b: ControllerButton): bool =
  for c in controllers:
    if c.btn(b):
      return true
  return false

proc btnp*(b: ControllerButton): bool =
  for c in controllers:
    if c.btnp(b):
      return true
  return false

proc jaxis*(axis: ControllerAxis, player: range[0..7] = 0): float =
  if player > controllers.high:
    return 0.0
...
#[
  Init or reset paletteMapDraw value
]#
proc pal*() =
  for i in 0..15:
    paletteMapDraw[i] = i
...
#[
  Set palatteTransparent color
]#
proc palt*(a: ColorId, trans: bool) =
  paletteTransparent[a] = trans

#[
  Init or reset palatteTransparent value
]#
proc palt*() =
  for i in 0..15:
    paletteTransparent[i] = if i == 0: true else: false
...
proc sget*(x, y: cint): ColorId =
  if x > spriteSheet.w - 1 or x < 0 or y > spriteSheet.h - 1 or y < 0:
    return 0
  var pixels = spriteSheet[].getPixels()
  return pixels[y * spriteSheet.pitch + x]
...
#[
  set all pixel to default color
]#
...
type LineIterator = iterator(): (cint, cint)
...
proc setFont(filename: string, chars: string): Font =
  return ft.setFont(swCanvas, filename, chars)
  ...
  if evt.kind == QuitEvent:
    ...
    for v in controllers:
      if v.sdlControllerId == evt.cdevice.which:
        return
    try:
      var controller = newController(evt.cdevice.which)
      controllers.add(newController(evt.cdevice.which))
    ...
    for controller in mitems(controllers):
      if controller.sdlControllerId == evt.cbutton.which:
        ...
        of SDL_CONTROLLER_BUTTON_LEFTSTICK:
          controller.setButtonState(pcLS, down)
        of SDL_CONTROLLER_BUTTON_RIGHTSTICK:
          controller.setButtonState(pcRS, down)
        of SDL_CONTROLLER_BUTTON_LEFTSHOULDER:
          controller.setButtonState(pcLB, down)
        of SDL_CONTROLLER_BUTTON_RIGHTSHOULDER:
          controller.setButtonState(pcRB, down)
    ...
    for controller in mitems(controllers):
      if controller.sdlControllerId == evt.caxis.which:
        ...
        of SDL_CONTROLLER_AXIS_LEFTX:
          controller.setAxisValue(pcLeftXAxis, value)
        of SDL_CONTROLLER_AXIS_LEFTY:
          controller.setAxisValue(pcLeftYAxis, value)
        of SDL_CONTROLLER_AXIS_RIGHTX:
          controller.setAxisValue(pcRightXAxis, value)
        of SDL_CONTROLLER_AXIS_RIGHTY:
          controller.setAxisValue(pcRightYAxis, value)
...
proc drawMap*(tx,ty, tw,th, dx,dy: int, scale: float = 1.0) =
...
proc getControllers*(): seq[Controller] =
  return controllers
...
proc init*(title: string = "Campfire") =
  ...
  # TODO: hard coding?
  writePath = $sdl2.getPrefPath("org.spicygee", "campfire")
  ...
  controllers = newSeq[Controller]()

  # add keyboard controller
  var keyboardController = newController(-1)
  controllers.add(keyboardController)

  # add mapping from file
  for i in 0..numJoysticks():
    if isGameController(i):
      try:
        var controller = newController(i)
  ...
  window = createWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, (screenWidth + screenPaddingX * 2) * screenScale, (screenHeight + screenPaddingY * 2) * screenScale, SDL_WINDOW_RESIZABLE)
